...
| Legacy analytics | Pass | React admin login mirrors the admin token into the legacy analytics session key, and the admin dashboard build now copies `web/analytics-dashboard/index.html` into `dist/legacy-analytics/index.html`. The proxy serves directory indexes, so both `/admin-dashboard/legacy-analytics/` and `/admin-dashboard/legacy-analytics/index.html` return the legacy dashboard HTML instead of the React SPA fallback. Primary operator workflow remains the authenticated React analytics UI. |
