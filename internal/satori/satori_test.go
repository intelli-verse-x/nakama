...
func TestSatoriClient_EventsPublish(t *testing.T) {
	...
	client := NewSatoriClient(ctx, logger, "<URL>", "<API KEY NAME>", "<API KEY>", "<SERVER KEY>", "<SIGNING KEY>", 0, 10_000, false, "", 0)
...
func TestSatoriClientMemory(t *testing.T) {
	...
	logger := NewConsoleLogger(os.Stdout, true)
	ctx := context.Background()
	client := NewSatoriClient(ctx, logger, "<SATORI_URL>", "<SATORI_API_KEY_NAME", "<SATORI_API_KEY>", "<SERVER KEY>", "<SIGNING_KEY>", 3600, 10_000, true, "", 0)
