...
function rpcTutorXRecordUsage(ctx, logger, nk, payload) {
    ...
    if (freeBeforeThisMsg <= 0) {
        ...
        if (!wallet) {
            // Fail-open on wallet read error (message allowed, nothing charged) but
            // log loudly — and never report a fake 0 balance to the client.
            utils.logError(logger, "tutorx_record_usage: wallet unavailable for user=" + userId + " — paid message allowed without charge");
        } else {
            // Balance check and deduction MUST use the same currency key (tutorxCurrencyKey)
            ...
            wallet.currencies[currencyKey] = currentBal - TUTORX_CONFIG.COST_PER_MESSAGE;
        ...
        // Same canonical key as check_allowance so the client never sees a different
        // number from this RPC than from the allowance check.
        try {
