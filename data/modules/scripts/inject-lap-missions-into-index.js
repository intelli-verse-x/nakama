'use strict';
/**
 * Surgical inject of quizverse_lap_missions_* into origin/master index.js
 * Usage: node data/modules/scripts/inject-lap-missions-into-index.js
 */
...
const SRC_MOD = path.join(ROOT, 'lap-missions', 'lap-missions.js');
...
const FN_MAP = {
  quizverse_lap_missions_get: 'rpcQuizverseLapMissionsGet',
  quizverse_lap_missions_upsert: 'rpcQuizverseLapMissionsUpsert',
};
...
function injectStubs(index) {
  if (index.indexOf('var __rpc_quizverse_lap_missions_get;') !== -1) {
    return index;
  }
  const anchor = 'var __rpc_quizverse_lap_gamification_upsert;';
  const fallback = 'var __rpc_quizverse_lap_library_stats;';
...
function injectRegisterCatchBlocks(index) {
  if (index.indexOf('__rpc_quizverse_lap_missions_get =') !== -1) {
    return index;
  }
...
function injectRegisterRpcCalls(index) {
  if (index.indexOf('registerRpc("quizverse_lap_missions_get"') !== -1) {
    return index;
  }
  const lines = RPC_IDS.map(function (id) {
    return (
...
function injectModuleBody(index, body) {
  if (index.indexOf('LAP_MISSIONS_COLLECTION') !== -1) {
    return index;
  }
  ...
  const banner =
    '\n// --- Module: lap-missions/lap-missions.js (injected) ---\n' +
    body +
    '\n';
...
function main() {
  ...
  console.log('[inject] Wrote ' + INDEX + ' (+' + RPC_IDS.length + ' missions RPCs)');
}
