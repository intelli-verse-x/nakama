...
namespace QvAgent {
  ...
  function bumpBucket(
    map: { [k: string]: { events: number; users: { [u: string]: boolean } } },
    key: string,
    uid: string
  ): void {
    if (!map[key]) map[key] = { events: 0, users: {} };
    map[key].events++;
    if (uid) map[key].users[uid] = true;
  }
  ...
  function lastBuckets(
    ...
    for (var i = start; i < keys.length; i++) {
      var b = map[keys[i]];
      var learners = 0;
      for (var u in b.users) { if (b.users.hasOwnProperty(u)) learners++; }
  ...
  function rpcPublicActivity(_ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, payload: string): string {
    try {
      ...
      var dayMap: { [k: string]: { events: number; users: { [u: string]: boolean } } } = {};
      var weekMap: { [k: string]: { events: number; users: { [u: string]: boolean } } } = {};
      var monthMap: { [k: string]: { events: number; users: { [u: string]: boolean } } } = {};
      var allUsers: { [u: string]: boolean } = {};
      var totalEvents = 0;

      var objs: any[] = [];
      try {
        var listRes = nk.storageList("", ACTIVITY_EVENTS_COLLECTION, maxScan);
        objs = (listRes && (listRes as any).objects) || [];
      } catch (e: any) {
      ...
      for (var i = 0; i < objs.length; i++) {
        ...
        if (gid && gid !== "quizverse" && gid !== QV_GAME_UUID) continue;
        ...
        if (uid) allUsers[uid] = true;
        totalEvents++;
      }

      var learnerCount = 0;
      for (var u2 in allUsers) { if (allUsers.hasOwnProperty(u2)) learnerCount++; }
      var dayCount = 0;
      for (var dk in dayMap) { if (dayMap.hasOwnProperty(dk)) dayCount++; }
