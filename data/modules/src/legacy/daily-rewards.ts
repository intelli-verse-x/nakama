  ...
  function rpcClaim(ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, payload: string): string {
    ...
    if (rewardAmount > 0) {
      try {
        ...
        var syncBody = JSON.stringify({ userId: userId, amount: rewardAmount, sourceType: "daily_reward", sourceId: "daily:day_" + status.day, description: "Daily reward day " + status.day });
