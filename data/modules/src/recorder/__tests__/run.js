#!/usr/bin/env node
// Harness for recorder_asr.test.ts.
//
//   cd data/modules
//   npx tsc -p tsconfig.tests.json && node src/recorder/__tests__/run.js
//
// tsconfig.tests.json is the production tsconfig with __tests__ included, so
// the tests compile into the same single-file bundle as the runtime and every
// namespace is reachable as a sandbox global. InitModule is stubbed out because
// running it would need a real Nakama initializer.
...
const suites = [
  ["RecorderAsrTests", sandbox.RecorderAsrTests],
];
