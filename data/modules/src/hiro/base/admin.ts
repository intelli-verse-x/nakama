namespace AdminConsole {
  ...
  function rpcAdminLiveEventsList(ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, payload: string): string {
    ...
    var seenCreatorIds: { [id: string]: boolean } = {};
    var creatorEventsCollection = "live_events";

    function pushCreatorLiveEventRow(obj: nkruntime.StorageObject): void {
      if (!obj.value) return;
      var ev: any = obj.value;
      var evId = String(ev.id || obj.key || "");
      if (!evId || seenCreatorIds[evId]) return;
      if (gameId && ev.gameId && ev.gameId !== gameId) return;
