...
namespace IdentityResolver {
  ...
  function isGhostBindingRecord(existing: any): boolean {
    if (!existing) return false;
    if (("" + (existing.confidence || "")).toLowerCase() === "low") return true;
    var src = "" + (existing.source || "");
    if (src.indexOf("conv_hub_") === 0) return true;
    if (src.indexOf("service_ghost") === 0) return true;
    return false;
  }

  function isGhostAccount(nk: nkruntime.Nakama, userId: string): boolean {
    try {
  ...
  function healStoredCognitoSub(
    ...
    if (!record || !record.cognito_sub) return record;
    var stored = "" + record.cognito_sub;
    var canonical = canonicalCognitoSub(nk, stored);
    if (!canonical || canonical === stored) return record;
    ...
    try {
      ...
      logger.warn(
        "identity heal failed: " + (err && err.message ? err.message : String(err)),
      );
  ...
  function isReplaceableGhostBinding(nk: nkruntime.Nakama, existing: any): boolean {
    if (!existing || !existing.cognito_sub) return false;
    if (isGhostBindingRecord(existing)) return true;
  ...
  function rpcResolveOrGhostCreate(ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, payload: string): string {
    try {
      ...
      if (existing && existing.cognito_sub) {
        ...
        var isGhost =
          isGhostBindingRecord(existing) || isGhostAccount(nk, existing.cognito_sub);
        return RpcHelpers.successResponse({
