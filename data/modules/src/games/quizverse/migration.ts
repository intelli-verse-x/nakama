...
namespace QuizVerseMigration {
  ...
  export var RPC_SUDOKU_GENERATE     = "quizverse_sudoku_generate";      // P7 Arcade Sudoku
  ...
  function isSudokuArrayLike(v: any): boolean {
    return v != null
      && typeof v === "object"
  ...
  function flattenSudokuGrid(grid: any): number[] | null {
    if (!isSudokuArrayLike(grid)) return null;
    ...
    if (grid.length === 81) {
      ...
      for (var i = 0; i < 81; i++) {
        ...
        if (flatCell === null || flatCell === undefined || flatCell === "") {
          ...
          if (!isFinite(flatN) || flatN < 0 || flatN > 9 || Math.floor(flatN) !== flatN) return null;
          flatOut.push(flatN);
        }
      }
      return flatOut;
    ...
    for (var r = 0; r < 9; r++) {
      var row = grid[r];
      if (!isSudokuArrayLike(row) || row.length !== 9) return null;
      for (var c = 0; c < 9; c++) {
        var cell = row[c];
        if (cell === null || cell === undefined || cell === "") {
          ...
          var n = Number(cell);
          if (!isFinite(n) || n < 0 || n > 9 || Math.floor(n) !== n) return null;
          out.push(n);
        }
      }
    }
  ...
  function rehydrateSudokuJson(value: any): any {
  ...
  function countSudokuFilled(grid: number[]): number {
    var n = 0;
